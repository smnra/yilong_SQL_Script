...
SELECT sdatetime,
      ...
      ,
       ...
       Round(decode(sum(M8021C0), 0, 0, 100 * sum(M8021C2) / 
             sum(M8021C0)),2) as 异频切换成功率, 
       sum(M8014C6+M8014C18+M8009C6-M8021C0) as 同频切换出执行,      
       sum(M8014C7+M8014C19+M8009C7-M8021C2) as 同频切换出成功次数,     
       Round(decode(sum(M8014C6+M8014C18+M8009C6-M8021C0), 0, 0, 100 * sum(M8014C7+M8014C19+M8009C7-M8021C2) / 
                     sum(M8014C6+M8014C18+M8009C6-M8021C0)),2) as 同频切换成功率,             
       ...
       sum(M8006C1) as ERAB建立成功个数,
        ...
        Round(decode(sum(M8006C1), 0, 0, 100 * sum(M8006C258-M8016C11) / 
                     sum(M8006C1)),2) as "倒流比",                       

       Round(decode(sum(M8014C6+M8014C18), 0, 0, 100 * sum(M8014C6) / 
                     sum(M8014C6+M8014C18)),2) as X2切换占比S1出准备,             
       Round(decode(sum(M8014C6+M8014C14), 0, 0, 100 * sum(M8014C6) / 
                     sum(M8014C6+M8014C14)),2) as X2切换占比S1出请求                       
