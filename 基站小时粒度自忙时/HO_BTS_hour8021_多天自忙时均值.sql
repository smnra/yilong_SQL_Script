...
SELECT &start_datetime||'_'||&end_datetime,
       --TTime,
       kkkk.区域,
       kkkk.bts_version,
       enb_id,
       ...
       Round(Decode(AVG(M8009C6 + M8014C6 + M8014C14),
                    0,
                    0,
                    100 * AVG(M8009C7 + M8014C7 + M8014C19) /
                    AVG(M8009C6 + M8014C6 + M8014C14)),
             2) LTE系统内切换成功率
      ,
       AVG(M8009C6) eNB内切换请求次数,
       AVG(M8009C7) eNB内切换成功次数,
       Round(decode(AVG(M8009C6), 0, 0, 100 * AVG(M8009C7) / AVG(M8009C6)),
             2) eNB内切换成功率,
       AVG(M8014C6) X2切换出请求次数,
       AVG(M8014C7) X2切换出成功次数,
       AVG(M8014C3+M8014C16) as ENB间切出无线,
       AVG(M8014C5+M8014C17) as ENB间切出传输,       
       Round(decode(AVG(M8014C6), 0, 0, 100 * AVG(M8014C7) / AVG(M8014C6)),
             2) X2切换成功率,
       AVG(M8014C14) as S1切换出请求次数,
       AVG(M8014C18) as eNB间S1出准备成功,
       AVG(M8014C19) as S1切换出成功次数,

       Round(decode(AVG(M8014C14),
                    0,
                    0,
                    100 * AVG(M8014C19) / AVG(M8014C14)),
             2) S1切换成功率,
       AVG(M8021C0) as 异频切换出执行,
       AVG(M8021C2) as 异频切换出成功次数,             
       Round(decode(AVG(M8021C0), 0, 0, 100 * AVG(M8021C2) / 
             AVG(M8021C0)),2) as 异频切换成功率, 
       AVG(M8014C6+M8014C18+M8009C6-M8021C0) as 同频切换出执行,      
       AVG(M8014C7+M8014C19+M8009C7-M8021C2) as 同频切换出成功次数,     
       Round(decode(AVG(M8014C6+M8014C18+M8009C6-M8021C0), 0, 0, 100 * AVG(M8014C7+M8014C19+M8009C7-M8021C2) / 
                     AVG(M8014C6+M8014C18+M8009C6-M8021C0)),2) as 同频切换成功率,             
       AVG(M8016C14) LTE至WCDMA切换请求次数,
       AVG(M8016C23) LTE至WCDMA切换成功次数,
       Round(decode(AVG(M8016C14),
                    0,
                    0,
                    100 * AVG(M8016C23) / AVG(M8016C14)),
             2) LTE至WCDMA切换成功率,
       AVG(M8016C11) LTE_CSFB_REDIR请求次数,
       decode((CASE WHEN kkkk.bts_version='FL16' THEN 'FL16' WHEN kkkk.bts_version='FLF16' THEN 'FL16' WHEN kkkk.bts_version='FL16A' THEN 'FL16' WHEN kkkk.bts_version='TL16' THEN 'FL16' ELSE kkkk.bts_version END),'FL16',AVG(M8006C258-M8016C11),AVG(M8006C15 - M8016C11)) as "重定向请求次数_NoCSFB",
       decode((CASE WHEN kkkk.bts_version='FL16' THEN 'FL16' WHEN kkkk.bts_version='FLF16' THEN 'FL16' WHEN kkkk.bts_version='FL16A' THEN 'FL16' WHEN kkkk.bts_version='TL16' THEN 'FL16' ELSE kkkk.bts_version END),'FL16',AVG(M8006C255+M8006C258+M8006C6+M8006C7+M8006C260),AVG(M8006C10 + M8006C15 + M8006C6 + M8006C7 +
       M8006C180)) as "E-RAB正常释放次数",
       AVG(M8006C1) as ERAB建立成功个数,
              decode((CASE WHEN kkkk.bts_version='FL16' THEN 'FL16' WHEN kkkk.bts_version='FLF16' THEN 'FL16' WHEN kkkk.bts_version='FL16A' THEN 'FL16' WHEN kkkk.bts_version='TL16' THEN 'FL16' ELSE kkkk.bts_version END),'FL16',(decode((AVG(M8006C255)+AVG(M8006C258)+AVG(M8006C6)+AVG(M8006C7)+AVG(M8006C260)),0,0,round((AVG(M8006C258) - AVG(M8016C11))/(AVG(M8006C255)+AVG(M8006C258)+AVG(M8006C6)+AVG(M8006C7)+AVG(M8006C260))*100,2))), (decode((AVG(M8006C10) + AVG(M8006C15) + AVG(M8006C6) + AVG(M8006C7) +
              AVG(M8006C180)),
              0,
              0,
              round((AVG(M8006C15) - AVG(M8016C11)) /
                    (AVG(M8006C10) + AVG(M8006C15) + AVG(M8006C6) +
                    AVG(M8006C7) + AVG(M8006C180)) * 100,
                    2)))) as "重定向比例",
                    
        Round(decode(AVG(M8006C1), 0, 0, 100 * AVG(M8006C258-M8016C11) / 
                     AVG(M8006C1)),2) as "倒流比",                       

       Round(decode(AVG(M8014C6+M8014C18), 0, 0, 100 * AVG(M8014C6) / 
                     AVG(M8014C6+M8014C18)),2) as X2切换占比S1出准备,             
       Round(decode(AVG(M8014C6+M8014C14), 0, 0, 100 * AVG(M8014C6) / 
                     AVG(M8014C6+M8014C14)),2) as X2切换占比S1出请求                       
  ...
  FROM (SELECT M8013.sdatetime ,M8013.TTime,
         ...
         WHERE M8013.cel_key_id = m8001.cel_key_id(+)
           ...
           AND M8013.cel_key_id = m8021.cel_key_id(+))kkkk
 GROUP BY &start_datetime||'_'||&end_datetime,
          --TTime,
          kkkk.bts_version,
          kkkk.区域,
          enb_id
