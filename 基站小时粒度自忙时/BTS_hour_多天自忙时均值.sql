...
FROM
    ...
    (SELECT 
        &start_datetime||'_'||&end_datetime AS DDATE,
        tab1.lncel_enb_id AS lncel_enb_id,
        --tab1.lncel_lcr_id AS lncel_lcr_id,
        --tab1.lnbts_name,
        tab1.区域,
        tab1.version,
        AVG(tab1.LTE小区可用率分子) AS LTE小区可用率分子,
        AVG(tab1.LTE小区可用率分母) AS LTE小区可用率分母,
        round(decode(AVG(tab1.LTE小区可用率分母),0,0,AVG(tab1.LTE小区可用率分子)/AVG(tab1.LTE小区可用率分母)*100),2) AS LTE小区可用率, 
        round(avg(tab1.LTE小区退服时长),2) AS LTE小区平均退服时长s,
        round(AVG(tab1.LTE小区退服时长),2) AS LTE小区总退服时长s
    ...
    (SELECT 
        ...
        tab2.区域,
        round((decode(AVG(tab2.RRC连接建立请求次数),0,0,AVG(tab2.RRC连接建立成功次数)/AVG(tab2.RRC连接建立请求次数)))*(decode(AVG(tab2.ERAB建立请求个数),0,0,AVG(tab2.ERAB建立成功个数)/AVG(tab2.ERAB建立请求个数)))*100,2) AS 无线接通率,
        round(decode(AVG(tab2.RRC连接建立请求次数),0,0,AVG(tab2.RRC连接建立成功次数)/AVG(tab2.RRC连接建立请求次数))*100,2) AS RRC连接建立成功率,
        AVG(tab2.RRC连接建立请求次数) AS RRC连接建立请求次数,
        AVG(tab2.RRC连接建立成功次数) AS RRC连接建立成功次数,
        AVG(tab2.Setup_comp_miss无应答) AS Setup_comp_miss无应答,
        AVG(tab2.Setup_comp_error小区拒绝) AS Setup_comp_error小区拒绝,
        AVG(tab2.Reject_RRM_RAC资源分配失) AS Reject_RRM_RAC资源分配失,
        AVG(tab2.SIGN_CONN_ESTAB_FAIL_MAXRRC) as SIGN_CONN_ESTAB_FAIL_MAXRRC,
        round(decode(AVG(tab2.ERAB建立请求个数),0,0,AVG(tab2.ERAB建立成功个数)/AVG(tab2.ERAB建立请求个数))*100,2) AS ERAB建立成功率,
        AVG(tab2.ERAB建立请求个数) AS ERAB建立请求个数,
        AVG(tab2.ERAB建立成功个数) AS ERAB建立成功个数,
        AVG(tab2.无线资源拥塞次数) AS 无线资源拥塞次数,
        AVG(tab2.传输资源拥塞次数) AS ERAB失败_传输层,
        AVG(tab2.ERAB建立失败_UE无响应) AS ERAB建立失败_UE无响应,    
        AVG(tab2.ERAB建立失败_配置) AS ERAB建立失败_安全模式, 
        ROUND(avg(tab2.E_RAB建立失败_核心网),0) AS E_RAB建立失败_核心网,
        ROUND(avg(tab2.E_RAB建立失败_无线层),0) AS E_RAB建立失败_无线层,
        round(decode(AVG(tab2.LTE业务释放次数),0,0,AVG(tab2.LTE业务掉线次数)/AVG(tab2.LTE业务释放次数))*100,2) AS LTE业务掉线率,
        AVG(tab2.LTE业务释放次数) AS LTE业务释放次数,
        AVG(tab2.LTE业务掉线次数) AS LTE业务掉线次数,
        AVG(tab2.EPC_EPS_BEARER_REL_REQ_RNL) AS MME释放的ERAB数网络层,
        AVG(tab2.ENB_EPS_BEARER_REL_REQ_RNL) AS eNB释放的ERAB数网络层,
        AVG(tab2.ENB_EPS_BEARER_REL_REQ_OTH) AS eNB释放的ERAB数other,
        AVG(tab2.ENB_EPS_BEARER_REL_REQ_TNL) AS eNB释放的ERAB数传输层,
        AVG(tab2.EPC_EPS_BEAR_REL_REQ_R_QCI1) AS  E_RAB异常释放_网络拥塞
    ...
    (SELECT 
        ...
        tab4.区域,
        round(AVG(tab4.空口上行业务字节数MB)/1024,2) AS 空口上行业务字节数MB,---MByte 单位
        round(decode(AVG(tab4.ACTIVE_TTI_UL),0,0,(AVG(tab4.空口上行业务字节数MB)*1000*8)/AVG(tab4.ACTIVE_TTI_UL)/1024),2) AS 空口上行业务平均速率,---Mbps单位
        round(AVG(tab4.空口下行业务字节数MB)/1024,2) AS 空口下行业务字节数MB,---MByte 单位
        round(decode(AVG(tab4.ACTIVE_TTI_DL),0,0,(AVG(tab4.空口下行业务字节数MB)*1000*8)/AVG(tab4.ACTIVE_TTI_DL)/1024),2) AS 空口下行业务平均速率---Mbps单位
    ...
    (SELECT 
        ...
        round(AVG(tab5.上行PRB可用数),2) AS 上行PRB可用总数,
        round(AVG(tab5.上行PRB占用平均数),2) AS 上行PRB占用总数,
        round(avg(tab5.上行PRB平均利用率)*100,2) AS 上行PRB平均利用率,
        round(avg(tab5.上行PRB占用平均数),2) AS 上行PRB占用平均数,
        round(AVG(tab5.下行PRB可用数),2) AS 下行PRB可用总数,
        round(AVG(tab5.下行PRB占用平均数),2) AS 下行PRB占用总数,
        round(avg(tab5.下行PRB平均利用率)*100,2) AS 下行PRB平均利用率,
        ...
        round(AVG(tab5.上行PRB资源使用个数),0) AS 上行PRB资源使用个数,
        round(AVG(tab5.下行PRB资源使用个数),0) AS 下行PRB资源使用个数, 
    ...
    (SELECT 
        ...
        round(AVG(tab6.平均RRC连接数_sum),2) 平均RRC连接数_sum,
        AVG(tab6.最大RRC连接数_sum) 最大RRC连接数_sum,
        AVG(tab6.最大RRC连接数_max) 最大RRC连接数_max,
        AVG(tab6.随机接入冲突解决次数) as 随机接入冲突解决次数,
        AVG(tab6.cell收随机前导数竞争) as cell收随机前导数竞争,
        AVG(tab6.cell收随机前导数非竞) as cell收随机前导数非竞,
        round(decode(AVG(tab6.cell收随机前导数竞争+tab6.cell收随机前导数非竞),0,0,AVG(tab6.随机接入冲突解决次数)/AVG(tab6.cell收随机前导数竞争+tab6.cell收随机前导数非竞))*100,2) AS PRACH成功率,
        round(AVG(tab6.平均激活用户数),0) AS 平均激活用户数,
        round(AVG(tab6.最大激活用户数),0) AS 最大激活用户数
    ...
    (SELECT 
        ...
        ROUND(AVG(tab7.RRC建立最大时延),0) AS RRC建立最大时延,
        round(decode(AVG(tab7.RRC重建次数),0,0,AVG(tab7.RRC重建成功次数)/AVG(tab7.RRC重建次数)*100),2) AS RRC重建成功率,
        AVG(tab7.RRC重建次数) AS RRC重建次数,
        AVG(tab7.RRC重建成功次数) AS RRC重建成功次数,
        AVG(tab7.RRC重建尝试因HO原因) AS RRC重建尝试因HO原因,
        AVG(tab7.RRC重建成功因HO原因) AS RRC重建成功因HO原因,
        AVG(tab7.RRC重建尝试因other原因) AS RRC重建尝试因other原因,
        AVG(tab7.RRC重建成功因other原因) AS RRC重建成功因other原因
    ...
    (SELECT 
        ...
        ROUND(AVG(tab8.E_RAB建立最大时延),0) AS E_RAB建立最大时延
    ...
    (SELECT 
        ...
        round(AVG(tab11.小区最大发射功率),0) AS 小区最大发射功率,
        round(decode(AVG(tab11.CQI大于等于10的比例_Y),0,0,AVG(tab11.CQI大于等于10的比例_X)/AVG(tab11.CQI大于等于10的比例_Y))*100,2) AS CQI大于等于10的比例,
        round(decode(AVG(tab11.CQI大于等于7的比例_Y),0,0,AVG(tab11.CQI大于等于7的比例_X)/AVG(tab11.CQI大于等于7的比例_Y))*100,2) AS CQI大于等于7的比例,
        round(decode(AVG(tab11.CQI小于等于6的比例_Y),0,0,AVG(tab11.CQI小于等于6的比例_X)/AVG(tab11.CQI小于等于6的比例_Y))*100,2) AS CQI小于等于6的比例,
        round(decode(AVG(tab11.CQI小于等于4的比例_Y),0,0,AVG(tab11.CQI小于等于4的比例_X)/AVG(tab11.CQI小于等于4的比例_Y))*100,2) AS CQI小于等于4的比例,
        round(decode(AVG(tab11.Average_CQI_Y),0,0,AVG(tab11.Average_CQI_X)/AVG(tab11.Average_CQI_Y)),2) AS Average_CQI,
        AVG(tab11.CQI_0) AS CQI_0,
        AVG(tab11.CQI_1) AS CQI_1,
        AVG(tab11.CQI_2) AS CQI_2,
        AVG(tab11.CQI_3) AS CQI_3,
        AVG(tab11.CQI_4) AS CQI_4,
        AVG(tab11.CQI_5) AS CQI_5,
        AVG(tab11.CQI_6) AS CQI_6,
        AVG(tab11.CQI_7) AS CQI_7,
        AVG(tab11.CQI_8) AS CQI_8,
        AVG(tab11.CQI_9) AS CQI_9,
        AVG(tab11.CQI_10) AS CQI_10,
        AVG(tab11.CQI_11) AS CQI_11,
        AVG(tab11.CQI_12) AS CQI_12,
        AVG(tab11.CQI_13) AS CQI_13,
        AVG(tab11.CQI_14) AS CQI_14,
        AVG(tab11.CQI_15) AS CQI_15
