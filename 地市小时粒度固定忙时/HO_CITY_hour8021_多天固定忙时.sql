...
select 
    ...
    TTime,
    区域,
    Round(Decode(sum(M8009C6 + M8014C6 + M8014C14),0, 0, 100 * sum(M8009C7 + M8014C7 + M8014C19) / sum(M8009C6 + M8014C6 + M8014C14)),2) LTE系统内切换成功率,
    sum(M8009C6) eNB内切换请求次数,
    sum(M8009C7) eNB内切换成功次数,
    Round(decode(sum(M8009C6), 0, 0, 100 * sum(M8009C7) / sum(M8009C6)),2) eNB内切换成功率,
    sum(M8014C6) X2切换出请求次数,
    sum(M8014C7) X2切换出成功次数,
    sum(M8014C3+M8014C16) as ENB间切出无线,
    sum(M8014C5+M8014C17) as ENB间切出传输,       
    Round(decode(sum(M8014C6), 0, 0, 100 * sum(M8014C7) / sum(M8014C6)),2) X2切换成功率,
    sum(M8014C14) as S1切换出请求次数,
    sum(M8014C18) as eNB间S1出准备成功,
    sum(M8014C19) as S1切换出成功次数,
    Round(decode(sum(M8014C14),0,0,100 * sum(M8014C19) / sum(M8014C14)),2) S1切换成功率,
    sum(M8021C0) as 异频切换出执行,
    sum(M8021C2) as 异频切换出成功次数,             
    Round(decode(sum(M8021C0), 0, 0, 100 * sum(M8021C2) / sum(M8021C0)),2) as 异频切换成功率, 
    sum(M8014C6+M8014C18+M8009C6-M8021C0) as 同频切换出执行,      
    sum(M8014C7+M8014C19+M8009C7-M8021C2) as 同频切换出成功次数,     
    Round(decode(sum(M8014C6+M8014C18+M8009C6-M8021C0), 0, 0, 100 * sum(M8014C7+M8014C19+M8009C7-M8021C2) /  sum(M8014C6+M8014C18+M8009C6-M8021C0)),2) as 同频切换成功率,             
    sum(M8016C14) LTE至WCDMA切换请求次数,
    sum(M8016C23) LTE至WCDMA切换成功次数,
    Round(decode(sum(M8016C14), 0, 0, 100 * sum(M8016C23) / sum(M8016C14)), 2) LTE至WCDMA切换成功率,
    sum(M8016C11) LTE_CSFB_REDIR请求次数,
    SUM(重定向请求次数_NoCSFB) as "重定向请求次数_NoCSFB",
    SUM(E_RAB正常释放次数) as "E_RAB正常释放次数",
    ROUND(DECODE(SUM(E_RAB正常释放次数),0,0,SUM(重定向请求次数_NoCSFB)/SUM(E_RAB正常释放次数))*100,2) as "重定向比例",
    sum(M8006C1) as ERAB建立成功个数,
    Round(decode(sum(M8006C1), 0, 0, 100 * sum(M8006C258-M8016C11) / sum(M8006C1)),2) as "倒流比",                       
    Round(decode(sum(M8014C6+M8014C18), 0, 0, 100 * sum(M8014C6) / sum(M8014C6+M8014C18)),2) as X2切换占比S1出准备,             
    Round(decode(sum(M8014C6+M8014C14), 0, 0, 100 * sum(M8014C6) / sum(M8014C6+M8014C14)),2) as X2切换占比S1出请求   
